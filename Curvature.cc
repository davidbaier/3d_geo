...
void Curvature::compute_normals_with_constant_weights() {
    ...
    OpenMesh::HalfedgeHandle he, start_halfedge_handle, ohe;
    OpenMesh::VertexHandle vh;
    Point p0,p1,p2;
    for(auto vertex :mesh_.vertices()){
        Point x_i (0.,0.,0.);
        p0 = mesh_.point(vertex);
        he =  mesh_.halfedge_handle(vertex);
        //Store first half edge to detect end of one-cycle around vertex
        start_halfedge_handle = he;
        vh = mesh_.to_vertex_handle(he);
        p1 = mesh_.point(vh);
        //Iterate until we have reached start_half_edge again
        do{
            ohe = mesh_.opposite_halfedge_handle(he);
            he = mesh_.next_halfedge_handle(ohe);
            vh = mesh_.to_vertex_handle(he);
            p2 = mesh_.point(vh);
            Point cp_norm = ((p2-p0)%(p1-p0)).normalize();
            auto alpha_1 = 1;
            x_i += alpha_1*cp_norm;
            p1 = p2;
        } while(he != start_halfedge_handle);
        // add normal to vertex_normal_ property.
        mesh_.property(vertex_normal_, vertex) = x_i.normalize();
    }
    // ------------- IMPLEMENT HERE ---------
    update_normals();
}
...
void Curvature::compute_normals_by_area_weights() {
    ...
    for(auto vertex :mesh_.vertices()){
        ...
        do{
            ...
            vh = mesh_.to_vertex_handle(he);
            p2 = mesh_.point(vh);
            //Compute Normal of current face.
            Point cross_product = ((p2-p0)%(p1-p0));
            //Compute the area weight
            double alpha_1 = cross_product.length()/2;
            Point cp_norm = cross_product.normalize();
            x_i += alpha_1*cp_norm;
            p1 = p2;
        } while(he != start_halfedge_handle);
        // add normal to vertex_normal_ property.
...
void Curvature::compute_normals_with_angle_weights() {
    ...
    for(auto vertex :mesh_.vertices()){
        ...
        do{
            ...
            Point l1 = (p1-p0);
            Point l2 = (p2-p0);
            //Compute weight based on angle between vectors.
            double alpha_1 = acos((l1 | l2)/(l1.length()*l2.length()));
            //Compute Normal of current face.
            Point cp_norm = (l2%l1).normalize();
            x_i += alpha_1 * cp_norm;
            p1 = p2;
        } while(he != start_halfedge_handle);
        // add normal to vertex_normal_ property.
